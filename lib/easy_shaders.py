...
def toGPUShape(shape, wrapMode=None, filterMode=None):
    ...
    glBufferData(
        GL_ARRAY_BUFFER, len(vertexData) * SIZE_IN_BYTES, vertexData, GL_STATIC_DRAW
    )
    ...
    if shape.textureFileName != None:
        ...
        textureSimpleSetup(
            gpuShape.texture, shape.textureFileName, wrapMode, filterMode
        )
    ...
    def __init__(self):
        ...
        self.shaderProgram = OpenGL.GL.shaders.compileProgram(
            ...
            OpenGL.GL.shaders.compileShader(
                fragment_shader, OpenGL.GL.GL_FRAGMENT_SHADER
            ),
        )
...
# Added for this work!
class SimpleTransformColorChangeShaderProgram:
    def __init__(self):
